...
## 부록: 원장의 `outcome` (2026-09-23)

과제가 어떻게 끝났는지(ADR-0010 부록)는 원장에 남고 span에는 넣지 않았다. 판단 기준은 이 ADR의 2번과
같다. span은 남의 collector로 나가고 원장은 이 기계에 남는다. `outcome`은 과제 하나의 결과이지 전달
경로의 성능이 아니므로, 물어야 할 곳은 `xsm status <과제 id>`다. 메시지 id가 둘을 잇는 것도 그대로다.
