...
| 2 | Claude | **뷰 하나만 낸다.** `xsm doc next`는 `leaves ∪ unverified`를 `read()` 순서 그대로 낸다. 점수 없음, 배정 없음, `--json`에 `"order": "created"`·`"assigns": false`. `wip` 노드는 후보에서 빠지고 부모에 주석 한 줄만 붙는다. 기존 뷰는 불변 | Orca가 자동 코디네이터를 폐기하고 뷰로 후퇴했다(orca-benchmark §2.4). A(점수)·B0(wip)·C′(deps)를 가르는 것은 서로 다른 순위 신호인데, 어느 신호도 읽지 않으면 어느 선택지도 채택되지 않는다. 사용자 결정(2026-09-23): "사실만 보여 주기" | **이것은 B0 채택이 아니다.** `wip`은 후보에서 빼는 데만 쓰이고 순서에는 닿지 않으며, `leaves`·`log`·`render`는 한 줄도 바뀌지 않았다(테스트로 고정). S10 결과가 오면 그때 A/B0/C′/D를 가른다. **상태는 Proposed 그대로다** |

| 3 | Claude | **S10 공동 편집은 버려진 줄로 선택지를 가르지 못했다.** 조건 5·6 각 6런(`docs/spikes/S10-results.md`). 중앙값 13.0 -> 8.5(1.53배)이지만 조건 6의 최대/최소 비가 17.0배로 파일럿 잡음 폭 7.1배를 넘는다. 사전 등록 §5 마지막 행("측정이 불안정, 결론을 내지 않는다")에 걸린다. **B1은 채택 근거를 얻지 못했다** | 3런씩일 때는 범위가 겹치지 않아(11~14 대 1~9) B1 채택으로 보였다. 3런을 더하자 5~20 대 1~17로 겹쳤다. 사전 등록이 아니었으면 그 우연을 근거로 채택했을 것이다 | 잡음 없이 갈린 것 둘은 남는다. (1) 조건 6은 6런 모두 `taking` 3~5건, 조건 5는 모두 0건 — 세션들은 강제 없이 규칙을 따른다. (2) 조건 6은 6런 모두 `xsm send` 0건, 조건 5는 2~8건 — 한 줄의 규칙이 소통 경로를 1:1에서 채널로 통째로 옮겼다. **무엇을 줄이는지는 아직 말할 수 없다.** 세션 5개·60분 재실행(약 6시간) 전까지 **상태는 Proposed 그대로다** |
