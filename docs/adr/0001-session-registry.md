...
## 부록: Codex 스레드 생존은 MCP 서버 생존이다 (2026-09-23)

S10 orch1에서 Codex TUI가 `/new`로 스레드를 바꿨는데 xsm은 옛 스레드를 계속 `live`로 보고, 그 앞으로 보낸 메시지 2건이 한 시간 동안 `queued`였다. 원인: Codex 생존 판정이 pid뿐이었고, Codex에는 `SessionEnd` 훅이 없으며, 한 프로세스가 스레드를 갈아탄다.

실측(Codex 0.155): TUI는 스레드를 열 때마다 MCP 서버 세트를 새로 띄우고, 떠난 스레드의 세트는 약 60초 뒤 죽인다. MCP 서버 `initialize`에는 스레드 id가 없고 환경변수도 비어 있어 MCP 서버가 자기 스레드를 알 수는 없다. 그래서 역할을 나눴다. MCP 서버는 비콘(`mcp/<pid>.json`)으로 "내가 살아 있다"만 말하고, 첫 프롬프트에 도는 훅이 스레드 id와 그때의 최신 비콘 pid(`mcp_pid`)를 묶는다. 판정은 §4.3: 비콘 pid가 죽으면 `ended(thread_replaced)`.

첫 프롬프트 전의 스레드는 여전히 주소가 없다(Codex가 아무것도 쓰지 않는다). 대신 `xsm list`가 비콘만 있는 스레드를 "열려 있으나 아직 주소 없음"으로 보여 준다. 사람이 그 창에 한 번 입력하거나 `/rename`하면 훅이 등록한다. 이 한계는 Codex 쪽 것이다.

이전 "결정 전 확인" 항목 중 "Codex `/new`·resume 후 `SessionStart` 재실행"은 확인됐다: 재실행되지 않는다(첫 프롬프트에만 훅이 돈다).
