...
| 라운드 | 참가자 | 입장 | 근거 | 반론/응답 |
|---|---|---|---|---|
| 3-구현 | 코디네이터 | A+E+L로 구현했다. 원장은 `queued`로 쓰고 수신 훅의 영수증으로만 `delivered`가 된다. 발신 전에 범위와 네이티브 보류를 예보한다 | `docs/xsm/PROTOCOL.md` 2·5장 | 샌드박스 Codex의 발신은 여전히 샌드박스 밖 실행이 필요하다(`sandbox-blocked`로 알린다) |
...
**A+E+L을 택한다**(사용자 확인: Codex는 턴 경계 전달로 충분하다, 2026-09-20. xsm v0.1로 구현).

- 전달은 네이티브 경로로 한다. Claude는 inbox 소켓, Codex는 `codex queue`다. 발신 결과는 전달이 아니다. 수신 훅의 기록만 `delivered`다.
- 수신 훅이 막을 때는 본문을 먼저 보관한다. 보관에 실패하면 경고를 붙여 통과시킨다.
- Codex는 진행 중인 턴에 끼어들지 않는다. PTY 주입(D)은 쓰지 않는다.
- **보강(ADR-0010).** xsm이 띄운 헤드리스 Codex 워커는 queue 대신 턴마다 잠깐 띄우는 `codex app-server`로 메시지를 받는다. 사용자가 연 Codex 세션에는 계속 queue를 쓴다. 초안의 "app-server를 쓰지 않는다"는 이미 열린 스레드에 두 번째 writer를 붙이지 않는다는 뜻으로 유지한다.

미실측: 샌드박스 Codex의 MCP 발신.
...
## 부록: 첫 프롬프트 전 Codex 스레드에 대기열을 직접 쓴다 (사용자 결정, 2026-09-23)

Codex TUI가 새 스레드를 열면 첫 프롬프트나 `/rename` 전까지 그 스레드는 스레드 행·rollout·훅 호출이 없다. 그래서 xsm이 등록할 수 없었고, `codex queue`도 `no rollout found`로 거부했다. 사람이 직접 연 Codex 창을 오케스트레이터로 쓰려면 사람이 먼저 아무거나 입력해야 했다(S10 orch1). 사용자는 이를 "연동이 부자연스럽고 막히는 지점"으로 꼽았다.

실측: (1) Codex 로그 DB는 스레드를 연 지 약 9초 안에 `pid:<pid>:…`와 스레드 id를 함께 기록한다. (2) `queued_items`에 `codex queue`와 같은 형식의 행을 직접 넣으면 쉬고 있는 TUI가 14초 안에 가져가 턴을 시작하고, 그 턴의 훅이 xsm 등록과 영수증을 정상으로 처리한다. xsm send로 종단 확인: 8초.

선택지는 셋이었다. A 대기열 직접 기록. B xsm이 띄운 창만 지원(Orca처럼 터미널을 소유해 키를 넣는다. 사람이 연 창은 여전히 막힌다). C 보류. 사용자는 A를 골랐다.

대가: Codex의 비공개 형식 두 가지(로그 DB의 `logs` 열, 대기열 DB의 `queued_items` 열)에 기댄다. 둘 다 쓰기 전에 모양을 확인하고, 다르면 추측하지 않고 실패한다(`codex-internal-changed` 또는 "id를 찾지 못함" 표시). 즉 Codex가 형식을 바꾸면 이 부록 이전 상태(첫 입력 필요)로 돌아갈 뿐 메시지를 잃지 않는다. `codex queue`가 성공하는 경우는 지금처럼 그것을 쓴다. 이 경로는 `no rollout found`일 때만 쓴다. 샌드박스 셸에서는 두 경로 모두 불가능하므로 MCP 서버가 보낸다.
