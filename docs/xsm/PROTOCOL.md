...
`config.json`의 `deny`는 세션 ref 목록이다. 발신 사전 검사에서는 발신자나 대상이 목록에 있으면 거부한다. 수신 게이트에서는 발신자나 수신자가 목록에 있으면 막는다. `xsm block <ref>`는 누구나 추가할 수 있고, `xsm unblock <ref>`는 사람 터미널에서만 된다.
...
- **환경.** 워커에는 `XSM_WORKER=<이름>`이 붙고, 부모의 `CLAUDE_CODE_SESSION_ID`·`CLAUDE_CODE_MESSAGING_SOCKET`과 프레임워크 패널 변수는 지운다. Claude 워커는 패널이든 헤드리스든 `--permission-mode default`로 뜨고, `--settings`로 `crossSessionInbound: accept`를 받고, 보고용으로 `xsm send`만 미리 허용된다(`--allowedTools Bash(<xsm> send:*)`). `spawn`·`stop`·`install` 같은 다른 xsm 명령은 여전히 승인을 거친다. 헤드리스 Codex 턴은 부른 세션의 환경을 물려받지 않고, spawn 때 저장한 최소 환경(`PATH`, `HOME`, 언어 설정 등)으로 돈다.
...
- **헤드리스 Codex.** 턴마다 `codex app-server`(stdio 줄 단위 JSON-RPC)를 띄워 한 턴을 돌고 닫는다. 순서는 `initialize`, `initialized`, 첫 턴이면 `thread/start` 아니면 `thread/resume`, 그다음 `turn/start`이고 `turn/completed`까지 읽는다. 스레드와 턴에는 매번 `approvalPolicy: on-request`, `sandbox: workspace-write`, 모델, 작업 폴더를 명시한다. 사용자 설정으로 돌아가지 않게 하려는 것이다. 이렇게 한 이유는 두 가지 실측이다. `exec resume`에는 `--sandbox`가 없어 재개 턴이 사용자 설정(`danger-full-access`)으로 돌았고, `codex exec`는 승인 정책을 `never`로 강제한다. xsm 훅(SessionStart·UserPromptSubmit)은 app-server에서도 돈다(실측). 서버가 보내는 승인 요청은 다음과 같이 답한다. 기다리는 동안 턴은 멈춰 있다.

  | 요청 | 허용 | 거부 |
  |---|---|---|
  | `item/commandExecution/requestApproval`, `item/fileChange/requestApproval` | `{"decision": "accept"}` | `{"decision": "decline"}` |
  | `execCommandApproval`, `applyPatchApproval` | `{"decision": "approved"}` | `{"decision": "denied"}` |
  | `item/permissions/requestApproval` | 요청한 `permissions` 그대로 | `{"permissions": {}}` |
  | `mcpServer/elicitation/request` | | `{"action": "decline"}` |
  | `item/tool/requestUserInput` | | `{"answers": {}}` |
  | 그 밖 | | JSON-RPC 오류 -32601 |

  메시지는 `inbox/`에 나노초 이름으로 쌓이고, 짧게 사는 pump(`xsm pump <이름>`)가 하나씩 처리한 뒤 끝난다. pump는 사는 동안 `pump.lock`에 `flock`을 쥐어 둘이 동시에 돌지 않게 한다. 항목은 `claimed/`로 옮겨 점유하고, 턴이 실패하면 되돌린다. 잠금을 놓은 뒤 수신함을 한 번 더 보고, 그사이 들어온 것이 있으면 다시 잡는다. 시작 턴에는 `--wait` 한도가 있다. 프롬프트가 메시지 자체라서 워커의 UserPromptSubmit 훅이 그대로 검문한다. 과제(`kind=task`)를 받은 턴이고 그 과제의 원장 상태가 `delivered`(워커의 검문 통과)일 때만 pump가 마지막 `agentMessage`를 워커 명의의 `reply`로 보낸다. 레지스트리는 워커 기록이 있는 동안 이 스레드를 `live`로 본다.
