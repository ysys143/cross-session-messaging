...
- 훅 명령에는 **파이썬 절대 경로**가 박힌다. 훅이 뜨지 못하면 검문이 통째로 열리기 때문이다(S8-g2). 기본값은 설치에 쓴 인터프리터이고, `--python`으로 고를 수 있다.
