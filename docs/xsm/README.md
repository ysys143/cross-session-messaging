...
- **auto 모드 세션에서는 허가 요청이 막힐 수 있다.** Claude Code auto 모드의 분류기가 `xsm_grant` 호출 자체를 권한 상승으로 보고 막았다(실측). 기본 권한 모드에서는 도구 호출을 사람에게 묻고 양식이 뜬다. auto 모드에서도 쓰려면 사용자가 권한 설정에서 `mcp__xsm__xsm_grant`를 허용해야 한다. xsm은 이 설정을 바꾸지 않는다.
